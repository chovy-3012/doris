...
public abstract class ExternalAclProvider {
    ...
    public static ExternalAclProvider getInstance() {
        if (singleton == null) {
            synchronized (ExternalAclProvider.class) {
                if (singleton == null) {
                    String cls = Config.external_acl_provider;
                    if (!StringUtils.isBlank(cls)) {
                        try {
                            singleton = (ExternalAclProvider) ClassUtils.getClass(cls).newInstance();
                        } catch (Exception e) {
                            e.printStackTrace();
    ...
    public abstract DbPrivTable getDbPrivTable();

    public abstract TablePrivTable getTablePrivTable();
}
